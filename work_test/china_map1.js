function foo() {
    ...
    var myChart_china_map = echarts.init(document.getElementById('china_and_child_map'));
    var myChart_china_child_map = echarts.init(document.getElementById('child_map'));
    ...
    //使用制定的配置项和数据显示图表
    myChart_china_map.setOption(optionMap);

    // 初始化echarts实例#ecf0f5
    myChart_china_map.on('click', function (params) {
        // $('#main').hide();
        // $('#main1').show();
        // $('#main_hist').show();
        $("#china_and_child_map").hide()
        $("#child_map").show();
        console.log(params.data.name + ':' + params.data.id)
        console.log($("#child_map").html()+"-------------------")
        // 柱状图
